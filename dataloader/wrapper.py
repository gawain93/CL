# -*- coding: utf-8 -*-
"""
Created on Mon Feb 24 14:13:47 2020
...
class Subclass(data.Dataset):
    ...
    Reorder the images in the dataset, following the class indexes
    Return: img, label
    """
    def __init__(self, dataset, class_list, remap=True):
        ...
        self.indices = []
        for c in class_list:
            self.indices.extend((dataset.labels==c).nonzero().flatten().tolist())   # find the index of each item according to the class label
        if remap:
    ...
    def __getitem__(self, index):
        img,target = self.dataset[self.indices[index]]            # self.indices[index] converts to original index
        if self.remap:
